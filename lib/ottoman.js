...
var MATH_OPERATION = ['average', 'min', 'max', 'sum'];
...
Ottoman.prototype._findModels = function (model, filter, options, callback) {
  ...
  schema.store.find('n1ql', mdlName, findOpts, function (err, res) {
    ...
    // special operation that require no ref look up
    // if contain math operation just return without referencing the id
    for(var mathIndex = 0; mathIndex < MATH_OPERATION.length; mathIndex++) {
      if (findOpts[MATH_OPERATION[mathIndex]] !== undefined) {
        callback(null, res);
        break;
      }
    }
    ...
    function handler() {
      ...
      if (proced === results.length) {
        ...
        if (findOpts.select !== undefined) {
          for (var m = 0; m < results.length; m++) {
            results[m] = _.pick(results[m], findOpts.select);
          }
        }
        callback(null, results);
...
Ottoman.prototype._buildModel = function (name, schemaDef, options) {
  ...
  model.convertId = ModelInstance.convertId;
