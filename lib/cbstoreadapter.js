...
CbStoreAdapter.prototype._ensureGsiIndices = function (callback) {
  ...
  queryPromise('CREATE PRIMARY INDEX ON `' + this.bucket._name + '` USING GSI')
    .then(function () {
      indexes.push('Ottoman_type');
      // Create ottoman type index, needed to make model lookups fast.
      return queryPromise('CREATE INDEX `Ottoman_type` ON `' +
    ...
    .then(function (value, result) {
      ...
      return Promise.map(indexes, function (index) {
        buildEm = 'BUILD INDEX ON `' + self.bucket._name + '`(`' + index + '`) USING GSI';
...
CbStoreAdapter.prototype._searchN1qlIndex =
  function (type, modelName, name, options, callback) {
    ...
    this.find(type, modelName, opts, callback);
  };
    ...
    if (filters.hasOwnProperty(i)) {
      ...
      if (_.isObject(filters[i])) {
        ...
        if (filters[i].$contains) {
          subfilters = filters[i].$contains;
          expressions.push('CONTAINS(' + ident + ',"' + subfilters + '")');
        }
        if (filters[i].$notIn) {
          if (_.isObject(filters[i].$in))
            subfilters = _.toArray(filters[i].$in);
          else
            subfilters = filters[i].$in;
          expressions.push(ident + 'NOT IN ' + '[' + subfilters + ']');
        }
        if (filters[i].$in) {
          ...
          expressions.push(ident + ' IN ' + '[' + subfilters + ']');
        }
        if (filters[i].$startsWith) {
          subfilters = filters[i].$startsWith;
          expressions.push('POSITION(' + ident + ',"' + subfilters + '") == 0');
        }
        if (filters[i].$endsWith) {
          subfilters = filters[i].$endsWith;
          expressions.push('POSITION(' + ident + ',"' + subfilters
                           + '") == (LENGTH(' + ident + ') - '
                           + subfilters.length + ')');
        }
        ...
        if (filters[i]['>']) {
          subfilters = filters[i]['>'];
          expressions.push(ident + ' > ' + subfilters);
        }
        if (filters[i]['<']) {
          subfilters = filters[i]['<'];
          expressions.push(ident + ' < ' + subfilters);
      ...
      if (filters[i].$like) {
        subfilters = filters[i].$like;
        expressions.push(ident + ' LIKE ' + '"' + subfilters + '"');
      }
...
CbStoreAdapter.prototype.find = function (type, modelName, options, callback) {
  ...
  if (options.ignoreCase && _.size(options.filter)) {
    _.each(options.filter, function(v,k){
      if (_.isString(v)) {
        options.filter['upper(' + k + ')'] = v.toUpperCase();
      } else if (v['$contains'] !== undefined) {
        options.filter['upper(' + k + ')'] = {
          '$contains' : v['$contains'].toUpperCase()
        }
      } else if (v['$startsWith'] !== undefined) {
        options.filter['upper(' + k + ')'] = {
          '$startsWith' : v['$startsWith'].toUpperCase()
        }
      } else if (v['$endsWith'] !== undefined) {
        options.filter['upper(' + k + ')'] = {
          '$endsWith' : v['$endsWith'].toUpperCase()
        }
      } else if (v['$like'] !== undefined) {
        options.filter['upper(' + k + ')'] = {
          '$like' : v['$like'].toUpperCase() //for some reason lodash remove %
        }
      }
  ...
  // Select query if there is arithmetic
  var findSubject = '';
  ...
  if (options.min !== undefined) {
    ...
    for (var iMin = 0; iMin < options.min.length; iMin++) {
      qsMin.push('MIN(' + options.min[iMin] + ') AS '
                 + options.min[iMin] + ' ');
    }

    findSubject += qsMin.join(',') + ' ,';
  }
  if (options.max !== undefined) {

    var qsMax = [];

    for (var iMax = 0; iMax < options.max.length; iMax++) {
      qsMax.push('MAX(' + options.max[iMax] + ') AS '
                 + options.max[iMax] + ' ');
    }

    findSubject += qsMax.join(',') + ' ,';
  }
  if (options.average !== undefined) {

    var qsAVG = [];

    for (var iAvg = 0; iAvg < options.average.length; iAvg++) {
      qsAVG.push('AVG(' + options.average[iAvg] + ') AS '
                 + options.average[iAvg] + ' ');
    }

    findSubject += qsAVG.join(',') + ' ,';
  }
  if (options.sum !== undefined) {

    var qsSUM = [];

    for (var iSum = 0; iSum < options.sum.length; iSum++ ) {
      qsSUM.push('SUM(' + options.sum[iSum] + ') AS '
                 + options.sum[iSum] + ' ');
    }

    findSubject += qsSUM.join(',') + ' ,';

  }

  if (options.groupBy !== undefined) {
    findSubject += options.groupBy.join(', ');
    groupByQs = ' GROUP BY[' + options.groupBy.join(',') + ']'
  }

  // default to original query if no operation dictate the subject
  if (!findSubject.length) {
    findSubject = 'META(b).id AS id';
  }

  if (findSubject.charAt(findSubject.length-1) === ',') {
    findSubject = findSubject.substr(0, findSubject.length - 1);
  }

  var fullQs = 'SELECT ' + findSubject + ' FROM `' + bucketName
                     + (/META\(b\)/.test(findSubject) ? '` b' : '`')
                     + whereQs + sortQs + groupByQs + pagingQs;
