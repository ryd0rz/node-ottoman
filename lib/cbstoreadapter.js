  ...
  for (var i in filters) {
    if (filters.hasOwnProperty(i)) {
      ...
      if (BOOLEAN.indexOf(i.toLowerCase()) !== -1) {
        ...
        var notExprs = [];
        if (Object.keys(filters[i]).length === 1) {
          _buildFilterExprs(filters[i], notExprs, '');
          expressions.push('NOT (' + notExprs.join(' AND ') + ')');
        } else {
          for (var e in filters[i]) {
            var singleNotFilter = { not: { [e]: filters[i][e] } };
            _buildFilterExprs(singleNotFilter, expressions, '');
          }
        }
...
CbStoreAdapter.prototype.count = function (type, modelName, options, callback) {
  ...
  if (expressions.length > 0) {
    whereQs = ' WHERE ' + expressions.join(' AND ') + " AND META(b).id NOT LIKE '_sync%' ";
  } else {
    whereQs = " WHERE META(b).id NOT LIKE '_sync%' ";
  }
