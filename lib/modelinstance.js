...
function convertId(mdlInstance) {
  if (mdlInstance._id !== undefined) {
    mdlInstance.id = mdlInstance._id;
    delete mdlInstance._id;
  }
...
ModelInstance.prototype._toCoo = function (refType, forceTyping) {
  ...
  if (objOut._id && !objOut.id) {
    objOut.id = objOut._id;
    delete objOut._id;
  }
...
ModelInstance.prototype.toJSON = function () {
  ...
  _.deepMapValues(val, function (value, path) {
    ...
    // { $ref: 'some-model-id', type: 'ModelName' }
    ...
    if (path.match(/\$ref$/)) {
      ...
      pathToRef.pop();
      var modelType = _.get(val, pathToRef.join('.') + '.type');
      refs[pathToRef.join('.')] = {
        $ref: modelType,
        $id: value
      };
...
/**
 * Returns a JSON serialized version of this model instance. and all the $ref
 *
 * The JSON serialized version is the same as the Coo, with certain internals
 * changed (references) so they are not exposed outside of the DB.
 *
 * @returns {Object}
 */
ModelInstance.prototype.toJsonAllRef = function () {
  if (! this.loaded()) {
    return null;
  }

  // retain the value of the reference if is loaded
  var refJson = {};
  _.each(this, function(value, key){
    if (value instanceof ModelInstance) {
      if (value.loaded()) {
        refJson[key] = value.toJSON();
      }
    }
  });
  ...
  var refs = {};
  if (!!refJson) {
    _.each(refJson, function (value, key){
      val[key] = value
    });
  }
...
ModelInstance.find = function (filter, options, callback) {
  this.schema.context._findModels(this, filter, options, function(err, data){
    if (err) {
      console.log('Error finding data' + err);
    } else {
      if (data.length) {
        for (var i = 0; i < data.length; i++) {
          data[i] = convertId(data[i]);
        }
      } else {
        data = convertId(data);
      }
